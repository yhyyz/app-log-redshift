...
class RealtimeLogRedshiftSink:
    ...
    def run_task(self, event_name_list, event_field_list, target_table,data_frame):
        task_status = {}
        try:
            self.logger("sync event_name_list info:" + str(event_name_list))
            target_table = target_table
            task_status["table_name"] = target_table
            event_name_reg = "|".join(event_name_list)
            df = data_frame.filter(gen_filter_udf(event_name_reg)(col('value')))
